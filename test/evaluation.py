...
# M1/M2/M3에서는 PyTorch가 알아서 MPS(GPU)를 사용하므로 'cpu'로 두어도 괜찮습니다.
DEVICE = "cpu"
...
try:
    ...
    print("인터넷 연결을 확인하거나, 모델 이름이 올바른지 다시 확인해주세요.")
    exit()
...
def analyze_prompt_effectiveness(original_prompt, augmented_prompt, original_music_path, augmented_music_path):
    ...
    try:
        ...
        # 오디오 파일 로드 및 전처리 (모델이 요구하는 48000Hz로 리샘플링)
        ...
        print("모델을 통해 유사도 점수를 계산합니다...")

        # === 여기가 수정된 핵심 로직입니다 ===
        with torch.no_grad():
            # 모델을 직접 호출하여 텍스트-오디오 간의 최종 유사도를 한번에 계산합니다.
            outputs = model(**inputs)
            # 최종 결과는 outputs.logits_per_text에 담겨 있습니다.
            # 이 행렬은 이미 정규화된 임베딩 간의 코사인 유사도 점수입니다.
            similarity_matrix = outputs.logits_per_text
        # ==================================

        # 점수 추출
        # similarity_matrix[i, j]는 i번째 텍스트와 j번째 오디오의 유사도입니다.
        ...
        print("-" * 30)
        # 점수에 100을 곱하면 백분율처럼 보기 편합니다.
        print(f"A. 원본 음악의 의도 부합도 (P_orig ↔ M_orig): {score_a * 100:.2f}점")
        print(f"B. 증강 음악의 원본 의도 부합도 (P_orig ↔ M_aug): {score_b * 100:.2f}점")
        print(f"C. 증강 음악의 디테일 반영도 (P_aug ↔ M_aug): {score_c * 100:.2f}점")
        print("-" * 30)
        ...
        print("[결론]")
        if improvement > 0:
            print(f"✅ 원본 의도 개선 효과: 증강 후 음악이 원본 의도에 더 부합합니다. (점수 {improvement * 100:+.2f}점 향상)")
        else:
            print(f"❌ 원본 의도 개선 효과: 증강이 원본 의도를 개선하지 못했습니다. (점수 {improvement * 100:+.2f}점)")

        if score_c * 100 > 70: # 임계값은 70점 등으로 조절 가능합니다.
             print(f"✅ 증강 내용 반영 효과: 증강 음악이 추가된 디테일을 성공적으로 반영했습니다. (점수: {score_c * 100:.2f}점)")
        else:
             print(f"⚠️ 증강 내용 반영 효과: 증강 음악이 추가된 디테일을 충분히 반영하지 못했을 수 있습니다. (점수: {score_c * 100:.2f}점)")
...
if __name__ == "__main__":
    ...
    m_orig_path = "ori.mp3" # 사용자님이 사용하신 파일 이름으로 변경
    m_aug_path = "aug.mp3" # 사용자님이 사용하신 파일 이름으로 변경
